<template>
   <main>
      ...
      <template v-else-if="item">
         ...
         <div
            id="breadcrumb"
            class="bg-forgotten-pink flex items-center px-[2vw] h-[50px] border-b-[3px] border-fandago mb-distance"
         >
            {{ `Home > Store > ${capitalize(item.category)} > ` }}
            <span class="font-bold inline-block ml-1">
               {{ item.name }}
            </span>
         </div>
         <!-- Product -->
         <div
            class="grid [grid:auto/30%_auto_20%] max-xl:[grid:auto/28%_auto_23%] max-lg:[grid:auto/30%_auto] max-xl:gap-fifth-distance gap-quarter-distance px-[2vw] mb-distance"
         >
            <!-- Image -->
            <div
               ...
               class="aspect-[calc(543/626)] border-4 flex justify-center items-center"
            >
               <AppImage
            ...
            <!-- Info -->
            <div class="px-distance max-lg:pr-0 h-max w-full overflow-hidden">
               <!-- Name -->
               <h1
                  class="font-oceanwide text-[24pt] w-[80%] max-2xl:w-[90%] max-lg:w-full max-xl:text-[17pt] mb-quarter-distance"
               >
                  {{ item.name }}
               </h1>
               <!-- Rating -->
               <div class="flex">
                  <img
                     ...
                     class="h-[30px] max-xl:h-[25px] max-xl:mr-2 mr-3"
                     v-for="_ in 5"
                     alt="*"
                  />
               </div>
               <!-- Price -->
               <div
                  class="my-half-distance max-xl:my-quarter-distance font-oceanwide flex items-center"
               >
                  <span
               ...
               <!-- Controls -->
               <div class="flex gap-[8.82px] my-quarter-distance">
                  <input
                     id="quantity"
                     type="number"
                     placeholder="Quantity"
                     class="h-[60px] w-[116px] max-xl:h-[45px] max-xl:w-[100px] pl-[10px] border-fandago border-4 max-xl:border-[3px]"
                  />
                  <button
                     id="addtocartbtn"
                     type="button"
                     class="bg-fandago font-oceanwide flex items-center justify-center px-[40px] max-xl:px-[20px] max-xl:h-[45px] gap-[9px] max-xl:gap-[5px] max-xl:text-[11pt]"
                  >
                     <svg
                        class="max-xl:scale-90"
                        width="26"
                        height="28"
                        viewBox="0 0 26 28"
                        fill="none"
                  ...
                  <button
                     id="wishlist-box"
                     type="button"
                     class="border-4 border-fandago max-xl:border-[3px] flex items-center justify-center h-[60px] w-[60px] max-xl:h-[45px] max-xl:w-[45px]"
                  >
                     <svg
                        class="max-xl:scale-75"
                        width="29"
                        height="26"
                        viewBox="0 0 29 26"
                        fill="none"
               ...
               <!-- Description -->
               <div
                  class="max-h-[350px] mb-half-distance max-lg:mt-distance overflow-hidden"
               >
                  <p
                     v-for="paragraph in item.description.split(/\n/g)"
                     class="paragraph text-justify text-ellipsis text-dark-purple text-[13pt] mb-distance max-xl:text-[10pt]"
                  >
                     {{ paragraph }}
                  </p>
               </div>
               <!-- Tags -->
               <div class="overflow-x-scroll scroll-hidden h-max w-full">
