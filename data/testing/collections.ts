export const collections: Collection[] = [
   {
      id: "1",
      name: "Comfort & Style",
      description:
         "Looking for clothing that is both comfortable and stylish? Our Comfort & Style collection is perfect for you! Our pieces are designed to keep you feeling comfortable all day long, while still looking fashionable and put-together. From cozy sweatpants to trendy tops, this collection has it all.",
      banner: "https://i.imgur.com/F74Ed6U.jpg",
      items: [],
      isFeatured: true,
   },
   {
      id: "2",
      name: "The Chic & Comfortable Edit",
      description:
         "Fashion shouldn't have to come at the expense of comfort! Our Chic & Comfortable Edit is full of pieces that will have you looking and feeling your best. From flowy dresses to soft sweaters, this collection has everything you need to create stylish and comfortable outfits for any occasion.",
      banner: "https://i.imgur.com/eJH2iA5.jpg",
      items: [],
      isFeatured: true,
   },
   {
      id: "3",
      name: "Effortlessly Stylish",
      description:
         "Effortlessly stylish? That's what you'll be with our latest collection! Our pieces are designed to be both comfortable and chic, allowing you to look put-together without any effort. From comfy leggings to stylish tops, this collection has everything you need to create effortless and stylish outfits.",
      banner: "https://i.imgur.com/bzNVT3x.jpg",
      items: [],
      isFeatured: false,
   },
   {
      id: "4",
      name: "The Comfortable Couture",
      description:
         "Comfort doesn't have to mean sacrificing style! Our Comfortable Couture collection features clothing that is both fashionable and comfortable. From soft blouses to cozy jeans, this collection has everything you need to create chic and comfortable outfits. So why not treat yourself to something from this collection and feel comfortable and stylish all at once?",
      banner: "https://i.imgur.com/PJtIPM7.jpg",
      items: [],
      isFeatured: false,
   },
   {
      id: "5",
      name: "The Comfortable Classic",
      description:
         "Classic style doesn't have to be uncomfortable! Our Comfortable Classic collection features clothing that is both timeless and comfortable. From simple dresses to elegant blouses, this collection has everything you need to create classic and comfortable outfits. So why not add a few pieces from this collection to your wardrobe and enjoy the best of both worlds?",
      banner: "https://i.imgur.com/wL4W4Dz.jpg",
      items: [],
      isFeatured: false,
   },
   {
      id: "6",
      name: "Comfortable & Cool",
      description:
         "Looking for clothing that is both comfortable and cool? Our Comfortable & Cool collection is perfect for you! Our pieces are designed to keep you feeling comfortable and looking trendy all at once. From soft t-shirts to cool jeans, this collection has it all. So why not give it a try and add a few pieces to your wardrobe today?",
      banner: "https://i.imgur.com/7sAO6Fe.jpg",
      items: [],
      isFeatured: false,
   },
];
